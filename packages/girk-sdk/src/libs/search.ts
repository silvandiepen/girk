// Search index generation — in-memory version
// Builds search shards and manifest without writing to filesystem
...
const getBodyLimit = (project: Project): number => {
  ...
  if (!Number.isFinite(configured) || configured <= 0) return SEARCH_BODY_LIMIT;
...
export const getSearchScope = (file: File, payload: Payload): SearchScope => {
  ...
  const archiveParent = getArchiveParent(file, payload.files);
  if (isSearchScope(archiveParent?.meta?.searchScope)) return archiveParent.meta.searchScope;
  if (isSearchScope(payload.project?.searchScope)) return payload.project.searchScope;
  return "project";
};
...
// SDK version: returns search files as data instead of writing to disk
export const buildSearchIndex = async (
  payload: Payload
): Promise<{ payload: Payload; files: Array<{ path: string; content: string; contentType: string }> }> => {
  ...
  if (!shouldGenerateSearch) return { payload, files };
  ...
  files.push({
    path: `/${SEARCH_DIRECTORY}/manifest.json`,
