...
/**
 * Build CSS from a provided base CSS string, injecting color variables.
 * This is the SDK-friendly version that doesn't read from the filesystem.
 */
export const buildCssFromContent = (baseCss: string, colors: ColorConfig | null): string => {
...
/**
 * Build CSS by reading from the pre-compiled CSS file.
 * In SDK mode, this uses a provided base CSS string.
 */
export const buildCss = async (colors: ColorConfig | null, baseCss?: string): Promise<string> => {
  if (baseCss) {
    return buildCssFromContent(baseCss, colors);
  }
  // Without a base CSS string, return just the color variables
  const lightMode = buildModeColorVariables(colors, "light");
  const darkMode = buildModeColorVariables(colors, "dark");
  return `:root { ${lightMode} } @media (prefers-color-scheme: dark) { :root { ${darkMode} } }`;
};
