...
export const RELEASE_NOTES_DIR = "apps/docs/release-notes";
...
${versions.map((version) => `- [${version}](/release-notes/${getVersionFile(version).replace(".md", "")}/index.html)`).join("\n")}
`;

export const writeReleaseNotes = ({
  rootDir,
  version,
  date,
  commits,
  ...
  const page = renderReleaseNotesPage({ version, date, sections });
